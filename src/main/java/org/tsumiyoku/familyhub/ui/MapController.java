...
public class MapController {
    ...
    private void reload() {
        try (Connection c = Database.get();
             PreparedStatement ps = c.prepareStatement("select id, name, latitude, longitude, note from places where user_id = ? AND deleted = 0 order by id desc")) {
    ...
    public void onAdd() {
        try (Connection c = Database.get();
             PreparedStatement ps = c.prepareStatement("insert into places(user_id, name, latitude, longitude, note) values(?, ?,?,?,?)")) {
            ps.setInt(1, Database.getCurrentUserId());
            extract(ps);
            ps.executeUpdate();
            reload();
    ...
    public void onUpdate() {
        ...
        try (Connection c = Database.get();
             PreparedStatement ps = c.prepareStatement("update places set name=?,latitude=?,longitude=?,note=? where id = ? and user_id = ?")) {
            extract(ps);
    ...
    private void extract(PreparedStatement ps) throws SQLException {
        ...
        ps.setDouble(3, Double.parseDouble(latField.getText().isBlank() ? "0" : latField.getText()));
        ps.setDouble(4, Double.parseDouble(lngField.getText().isBlank() ? "0" : lngField.getText()));
    ...
    public void onDelete() {
        ...
        try (Connection c = Database.get();
             PreparedStatement ps = c.prepareStatement("UPDATE places SET deleted=1 WHERE id = ? AND user_id = ?")) {
    ...
    public void onOpenMap() {
        try {
            ...
            String url = String.format("https://www.openstreetmap.org/?mlat=%f&mlon=%f#map=15/%f/%f", lat, lng, lat, lng);
            if (java.awt.Desktop.isDesktopSupported()) {
                java.awt.Desktop.getDesktop().browse(new java.net.URI(url));
            }
        } catch (Exception e) {
            org.tsumiyoku.familyhub.util.Dialogs.error("Carte", "Coordonnées invalides ou navigateur indisponible", e);
        }
    }
}
