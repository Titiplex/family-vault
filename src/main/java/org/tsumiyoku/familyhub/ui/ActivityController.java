...
public class ActivityController {
    ...
    private void reload() {
        try (Connection c = Database.get();
             PreparedStatement ps = c.prepareStatement("select id, date, description, duration_minutes from activities where user_id = ? AND deleted = 0 order by id desc")) {
    ...
    public void onAdd() {
        try (Connection c = Database.get();
             PreparedStatement ps = c.prepareStatement("insert into activities(user_id, date, description, duration_minutes) values(?, ?,?,?)")) {
            ps.setInt(1, Database.getCurrentUserId());
            ps.setString(2, dateField.getValue() == null ? null : dateField.getValue().toString());
            ps.setString(3, descArea.getText());
            ps.setInt(4, Integer.parseInt(durationField.getText().isBlank() ? "0" : durationField.getText()));
            ps.executeUpdate();
            reload();
            onClear();
        } catch (Exception e) {
    ...
    public void onUpdate() {
        ...
        try (Connection c = Database.get();
             PreparedStatement ps = c.prepareStatement("update activities set date=?,description=?,duration_minutes=? where id = ? and user_id = ?")) {
            ps.setString(2, dateField.getValue() == null ? null : dateField.getValue().toString());
            ps.setString(3, descArea.getText());
            ps.setInt(4, Integer.parseInt(durationField.getText().isBlank() ? "0" : durationField.getText()));
    ...
    public void onDelete() {
        ...
        try (Connection c = Database.get();
             PreparedStatement ps = c.prepareStatement("UPDATE activities SET deleted=1 WHERE id = ? AND user_id = ?")) {
