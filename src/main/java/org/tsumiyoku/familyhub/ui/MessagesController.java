...
public class MessagesController {
    ...
    public void initialize() {
        col1.setText("À");
        col2.setText("Sujet");
        col3.setText("Contenu");
        col4.setText("Horodatage");
    ...
    private void reload() {
        try (Connection c = Database.get();
             PreparedStatement ps = c.prepareStatement("select id, recipient, subject, content, timestamp from messages where user_id = ? AND deleted = 0 order by id desc")) {
    ...
    public void onAdd() {
        try (Connection c = Database.get();
             PreparedStatement ps = c.prepareStatement("insert into messages(user_id, recipient, subject, content, timestamp) values(?, ?,?,?,?)")) {
            ps.setInt(1, Database.getCurrentUserId());
            ps.setString(2, recipientField.getText());
            ps.setString(3, subjectField.getText());
            ps.setString(4, contentArea.getText());
            ps.setString(5, timestampField.getText().isBlank() ? java.time.OffsetDateTime.now().toString() : timestampField.getText());
            ps.executeUpdate();
            reload();
            onClear();
        } catch (Exception e) {
    ...
    public void onUpdate() {
        ...
        try (Connection c = Database.get();
             PreparedStatement ps = c.prepareStatement("update messages set recipient=?,subject=?,content=?,timestamp=? where id = ? and user_id = ?")) {
            ps.setString(2, recipientField.getText());
            ps.setString(3, subjectField.getText());
            ps.setString(4, contentArea.getText());
            ps.setString(5, timestampField.getText().isBlank() ? java.time.OffsetDateTime.now().toString() : timestampField.getText());
    ...
    public void onDelete() {
        ...
        try (Connection c = Database.get();
             PreparedStatement ps = c.prepareStatement("UPDATE messages SET deleted=1 WHERE id = ? AND user_id = ?")) {
