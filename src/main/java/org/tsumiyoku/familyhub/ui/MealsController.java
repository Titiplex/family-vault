...
public class MealsController {
    ...
    private void reload() {
        try (Connection c = Database.get();
             PreparedStatement ps = c.prepareStatement("select id, date, meal, calories from meals where user_id = ? AND deleted = 0 order by id desc")) {
    ...
    public void onAdd() {
        try (Connection c = Database.get();
             PreparedStatement ps = c.prepareStatement("insert into meals(user_id, date, meal, calories) values(?, ?,?,?)")) {
            ps.setInt(1, Database.getCurrentUserId());
            ps.setString(2, dateField.getValue() == null ? null : dateField.getValue().toString());
            ps.setString(3, mealField.getText());
            ps.setInt(4, Integer.parseInt(calField.getText().isBlank() ? "0" : calField.getText()));
            ps.executeUpdate();
            reload();
            onClear();
        } catch (Exception e) {
    ...
    public void onUpdate() {
        ...
        try (Connection c = Database.get();
             PreparedStatement ps = c.prepareStatement("update meals set date=?,meal=?,calories=? where id = ? and user_id = ?")) {
            ps.setString(2, dateField.getValue() == null ? null : dateField.getValue().toString());
            ps.setString(3, mealField.getText());
            ps.setInt(4, Integer.parseInt(calField.getText().isBlank() ? "0" : calField.getText()));
    ...
    public void onDelete() {
        ...
        try (Connection c = Database.get();
             PreparedStatement ps = c.prepareStatement("UPDATE meals SET deleted=1 WHERE id = ? AND user_id = ?")) {
