...
public final class PeerStore {
    public record Peer(String deviceId, String publicKey, String address, int port, String pairingSecretHash) {
    }

    public static void addPeer(Peer p) throws Exception {
        try (Connection c = Database.get();
             PreparedStatement ps = c.prepareStatement(
                     "INSERT OR REPLACE INTO trusted_peers(device_id, public_key, address, port, pairing_secret_hash, added_at) VALUES(?,?,?,?,?,strftime('%Y-%m-%dT%H:%M:%fZ','now'))")) {
    ...
    public static List<Peer> list() throws Exception {
        try (Connection c = Database.get();
             ResultSet rs = c.createStatement().executeQuery("SELECT device_id, public_key, address, port, pairing_secret_hash FROM trusted_peers ORDER BY added_at DESC")) {
            List<Peer> out = new ArrayList<>();
            while (rs.next())
                out.add(new Peer(rs.getString(1), rs.getString(2), rs.getString(3), rs.getInt(4), rs.getString(5)));
            return out;
        }
    }
