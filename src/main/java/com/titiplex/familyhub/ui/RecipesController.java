...
public class RecipesController {
    ...
    private void reload() {
        try (Connection c = Database.get();
             PreparedStatement ps = c.prepareStatement("select id, title, ingredients, steps from recipes where user_id = ? AND deleted = 0 order by id desc")) {
            ContactsController.extractUser(ps, table);
        } catch (Exception e) {
    ...
    public void onAdd() {
        try (Connection c = Database.get();
             PreparedStatement ps = c.prepareStatement("insert into recipes(user_id, title, ingredients, steps) values(?, ?,?,?)")) {
            ps.setInt(1, Database.getCurrentUserId());
            ps.setString(2, titleField.getText());
            ps.setString(3, ingredientsArea.getText());
            ps.setString(4, stepsArea.getText());
            ps.executeUpdate();
            reload();
            onClear();
        } catch (Exception e) {
    ...
    public void onUpdate() {
        ...
        try (Connection c = Database.get();
             PreparedStatement ps = c.prepareStatement("update recipes set title=?,ingredients=?,steps=? where id = ? and user_id = ?")) {
            ps.setString(2, titleField.getText());
            ps.setString(3, ingredientsArea.getText());
            ps.setString(4, stepsArea.getText());
    ...
    public void onDelete() {
        ...
        try (Connection c = Database.get();
             PreparedStatement ps = c.prepareStatement("UPDATE recipes SET deleted=1 WHERE id = ? AND user_id = ?")) {
