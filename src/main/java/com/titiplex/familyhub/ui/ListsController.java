...
public class ListsController {
    ...
    public void initialize() {
        col1.setText("Titre");
        col2.setText("Terminé");
        col3.setText("Échéance");
        col4.setText("");
    ...
    private void reload() {
        try (Connection c = Database.get();
             PreparedStatement ps = c.prepareStatement("select id, title, done, due_date from lists where user_id = ? AND deleted = 0 order by id desc")) {
    ...
    public void onAdd() {
        try (Connection c = Database.get();
             PreparedStatement ps = c.prepareStatement("insert into lists(user_id, title, done, due_date) values(?, ?,?,?)")) {
            ps.setInt(1, Database.getCurrentUserId());
            ps.setString(2, titleField.getText());
            ps.setInt(3, (doneCheck.isSelected()) ? 1 : 0);
            ps.setString(4, dueDate.getValue() == null ? null : dueDate.getValue().toString());
            ps.executeUpdate();
            reload();
            onClear();
        } catch (Exception e) {
    ...
    public void onUpdate() {
        ...
        try (Connection c = Database.get();
             PreparedStatement ps = c.prepareStatement("update lists set title=?,done=?,due_date=? where id = ? and user_id = ?")) {
            ps.setString(2, titleField.getText());
            ps.setInt(3, (doneCheck.isSelected()) ? 1 : 0);
            ps.setString(4, dueDate.getValue() == null ? null : dueDate.getValue().toString());
    ...
    public void onDelete() {
        ...
        try (Connection c = Database.get();
             PreparedStatement ps = c.prepareStatement("UPDATE lists SET deleted=1 WHERE id = ? AND user_id = ?")) {
