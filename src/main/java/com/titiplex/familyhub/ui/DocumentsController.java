...
public class DocumentsController {
    ...
    private void reload() {
        try (Connection c = Database.get();
             PreparedStatement ps = c.prepareStatement("select id, path, title, tags from documents where user_id = ? AND deleted = 0 order by id desc")) {
    ...
    public void onAdd() {
        try (Connection c = Database.get();
             PreparedStatement ps = c.prepareStatement("insert into documents(user_id, path, title, tags) values(?, ?,?,?)")) {
            ps.setInt(1, Database.getCurrentUserId());
            ps.setString(2, pathField.getText());
            ps.setString(3, titleField.getText());
            ps.setString(4, tagsField.getText());
            ps.executeUpdate();
            reload();
            onClear();
        } catch (Exception e) {
    ...
    public void onUpdate() {
        ...
        try (Connection c = Database.get();
             PreparedStatement ps = c.prepareStatement("update documents set path=?,title=?,tags=? where id = ? and user_id = ?")) {
            ps.setString(2, pathField.getText());
            ps.setString(3, titleField.getText());
            ps.setString(4, tagsField.getText());
    ...
    public void onDelete() {
        ...
        try (Connection c = Database.get();
             PreparedStatement ps = c.prepareStatement("UPDATE documents SET deleted=1 WHERE id = ? AND user_id = ?")) {
